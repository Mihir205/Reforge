...
class TestPhase4Features:
    ...
    def test_report_and_ticket_generation(self, tmp_path):
        ...
        assert report_data["metrics"]["total_files"] >= 1
        assert "report_test.html" in report_data["escalated_files"]
        
        # Check review ticket
        safe_name = "report_test.html".replace("/", "_").replace("\\", "_")
        ticket_path = Path(f"reports/review_tickets/REVIEW_{safe_name}.md")
        ...
        assert "Secret detected" in ticket_content
